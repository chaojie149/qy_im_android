...
public abstract class BaseBuilder {
    ...
    public BaseCall rebuild() {
        // 重新计算验参，,
        params.remove("secret");
        return build(mac, beforeLogin);
    }
    ...
    public class BaseCall {
        ...
        public Call execute(AbstractCallback<?> callback) {
            callback.attach(BaseBuilder.this);
        ...
        @WorkerThread
        public void refreshAccessTokenSync(AbstractCallback<?> callback) {
            callback.attach(BaseBuilder.this);
            if (checkCrashed(callback)) {
                return;
            }
