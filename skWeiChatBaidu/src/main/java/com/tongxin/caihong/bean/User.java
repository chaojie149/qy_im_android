...
public class User implements Parcelable, Cloneable {
    ...
    private String showMarker; // 聊天背景显示水印内容，
    private String toShowMarker; // 给对方设置的聊天背景水印内容，
    ...
    public String getTelephoneNoAreaCode() {
        String areaCode = String.valueOf(UserSp.getInstance(MyApplication.getContext(), userId).getAreaCode());
        // 删除开头的区号，
        if (!TextUtils.isEmpty(telephone) && telephone.startsWith(areaCode)) {
            telephone = telephone.substring(areaCode.length());
        }
        if (!TextUtils.isEmpty(telephone) && telephone.startsWith("86")) {
            telephone = telephone.substring(areaCode.length());
        }
        return telephone;
    }
    ...
    public String getToShowMarker() {
        return toShowMarker;
    }

    public void setToShowMarker(String toShowMarker) {
        this.toShowMarker = toShowMarker;
    }
