...
public class ContactsUtil {
    ...
    public static Map<String, Contacts> getPhoneContacts(Context context) {
        ...
        Cursor cursor = null;
        try {
            cursor = context.getContentResolver().query(uri, projects, null, null, null);
        } catch (StaleDataException ignored) {
            // vivo手机可能抛异常， android.database.StaleDataException:Attempted to access a cursor after it has been closed.
            // 联系人也不是必须的，有权限读不出来基本都是国产魔改系统限制了，就当成没有联系人，
        }
        ...
        try {
            ...
            String mobilePrefix = PreferenceUtils.getString(MyApplication.getContext(), Constants.AREA_CODE_KEY);
