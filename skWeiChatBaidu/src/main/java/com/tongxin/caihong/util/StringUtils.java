...
import static com.tongxin.caihong.bean.message.XmppMessage.TYPE_CHAT_HISTORY;
import static com.tongxin.caihong.bean.message.XmppMessage.TYPE_IMAGE_TEXT;
import static com.tongxin.caihong.bean.message.XmppMessage.TYPE_IMAGE_TEXT_MANY;
...
*/
     ...
     * todo ChatMessage: getSimpleContent(use for MessageFragment relay)
    ...
    public static String getMessageContent(ChatMessage chatMessage, boolean isShowName) {
        ...
        if (type == XmppMessage.TYPE_TEXT
        ...
        } else if (type == XmppMessage.TYPE_REQUEST_COMMENT) {
            context = "[" + MyApplication.getInstance().getString(R.string.service_title_grade) + "]";
        } else if (type == XmppMessage.TYPE_SERVICE_FORWARD) {
            context = "[" + MyApplication.getInstance().getString(R.string.service_forward) + "]";
        } else if (type == XmppMessage.TYPE_SERVICE_MENU) {
            context = "[" + MyApplication.getInstance().getString(R.string.service_menu) + "]";
            ...
            context = MyApplication.getContext().getString(R.string.type_meeting_invite);
        } else if (type == XmppMessage.TYPE_LIVE_INVITE) {
            context = MyApplication.getContext().getString(R.string.type_live_invite);
        } else if (type == XmppMessage.TYPE_INVITE_JOIN_ROOM) {
            context = MyApplication.getContext().getString(R.string.type_invite_join_room);
        } else if (type == XmppMessage.TYPE_COMPANY_INVITE_JOIN) {
            context = MyApplication.getContext().getString(R.string.type_invite_join_company);
        } else if (type == XmppMessage.TYPE_APPLET) {
            context = MyApplication.getContext().getString(R.string.type_applet);
        } else if (type == XmppMessage.TYPE_SHARE) {
            context = getShareContent(chatMessage.getContent());
        ...
        } else if (type == TYPE_IMAGE_TEXT || type == XmppMessage.TYPE_IMAGE_TEXT_HTML || type == TYPE_IMAGE_TEXT_MANY) {
            ...
            context = "[" + MyApplication.getInstance().getString(R.string.group_bulletin) + "]";
        } else if (type == XmppMessage.TYPE_MUCFILE_ADD) {
            context = "[" + MyApplication.getInstance().getString(R.string.group_file) + "]";
        } else if (type >= XmppMessage.TYPE_IS_CONNECT_VOICE
                && type <= XmppMessage.TYPE_IS_MU_START_CONNECT_LIVE) {
            context = getAudioMessageContent(chatMessage);
    ...
    public static String getShareContent(String shareContent) {
        String context;
        try {
            String[] splitContent = shareContent.split("\\{");
            Share share = null;
            if (!TextUtils.isEmpty(splitContent[0])) {
                share = Share.json2Share("{" + splitContent[1]);
            } else {
                share = Share.json2Share(shareContent);
            }
            context = MyApplication.getContext().getString(R.string.share_from_people, share.getPublisherName());
/*
            if (!TextUtils.isEmpty(splitContent[0])) {
                context = splitContent[0] + MyApplication.getContext().getString(R.string.share_from_people, share.getPublisherName());
            } else {
                context = MyApplication.getContext().getString(R.string.share_from_people, share.getPublisherName());
            }
*/
        } catch (Exception e) {
            // 兼容ios把content替换成普通文本的情况，
    ...
    public static String getAudioMessageContent(ChatMessage chatMessage) {
        ...
        } else if (type == XmppMessage.TYPE_IS_MU_CONNECT_LIVE) {
            content = MyApplication.getContext().getString(R.string.tip_invite_live_meeting);
    ...
    public static String getTimeLengthString(long timeLen) {
        return getTimeLengthStringFromMillis(TimeUnit.SECONDS.toMillis(timeLen));
    }

    /**
     * 统一时间段转字符串最终调用都在这里，
     *
     * @param timeLenMillis 单位毫秒，
     * @return
     */
    @SuppressLint("SimpleDateFormat")
    public static String getTimeLengthStringFromMillis(long timeLenMillis) {
        SimpleDateFormat sdf;
        if (timeLenMillis < TimeUnit.HOURS.toMillis(1)) {
            sdf = new SimpleDateFormat("mm:ss");
        } else {
            sdf = new SimpleDateFormat("HH:mm:ss");
        }
        // 这里sdf用于转换时间段而不是时间点，设置0时区避免奇葩时区影响，比如新加坡+8区会在转换时再加30分钟，
        sdf.setTimeZone(TimeZone.getTimeZone("GMT+0:00"));
        return sdf.format(new Date(timeLenMillis));
    }
    ...
    public static String toPercent(String doubleText){
