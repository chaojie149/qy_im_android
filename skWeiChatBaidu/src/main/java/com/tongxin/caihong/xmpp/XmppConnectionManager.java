...
public class XmppConnectionManager {
    ...
    private Handler mNotifyConnectionHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            Log.e(TAG, "当前XMPP连接状态:" + mXMPPCurrentState);
    ...
    private BroadcastReceiver mNetWorkChangeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
                Log.e(TAG, "ConnectivityManager.CONNECTIVITY_ACTION-->HashCode" + this.hashCode());
                if (mIsNetWorkActive) {
                    Log.e(TAG, "有网络连接1");
                    if (System.currentTimeMillis() - mLastLoginTime >= 1000) {
                        mLastLoginTime = System.currentTimeMillis();
                        if (isInValid) {
                            Log.e(TAG, "isInValid == true，断开连接在登录1");
                            isInValid = false;
                            // Socket断开连接，即失效了，直接调用login没有用，需要先disconnect在登录(即建立一个Socket新的连接)
                            mConnection.disconnect();
                            login();
                        } else {
                            Log.e(TAG, "isInValid == false，直接登录1");
                            // TODO: 应该判断socket已经连接上了才能login, 否则导致“连接中”变成“离线”，
                            login();
                        }
                    } else {
                        Log.e(TAG, "有网络连接且连续回调到该广播1");
                    }
                } else {
                    Log.e(TAG, "无网络连接1");
                }
            } else {
                Log.e(TAG, "SocketException.SELECTION_KEY_INVALID-->HashCode" + this.hashCode());
                if (mIsNetWorkActive) {
                    Log.e(TAG, "有网络连接2");
                    if (isInValid) {
                        Log.e(TAG, "isInValid == true，断开连接在登录2");
                        isInValid = false;
                        // Socket断开连接，即失效了，直接调用login没有用，需要先disconnect在登录(即建立一个Socket新的连接)
                        mConnection.disconnect();
                        login();
                    } else {
                        Log.e(TAG, "isInValid == false，已经处理过了，不处理2");
                    }
                } else {
                    Log.e(TAG, "无网络连接2");
                }
    ...
    public XmppConnectionManager(Context context, NotifyConnectionListener listener) {
        ...
        mConnection.addConnectionListener(new NotifyConnectionListener() {
            ...
            public void notifyConnectionClosedOnError(String err) {
                Log.e(TAG, "notifyConnectionClosedOnError：" + err);
    ...
    public void addRead(ReadManager.MucReadItem item) {
        mConnection.addRead(item);
    }

    public synchronized void login() {
        if (mConnection.isAuthenticated()) {
            Log.e(TAG, "已认证，Return");
            return;
        }
