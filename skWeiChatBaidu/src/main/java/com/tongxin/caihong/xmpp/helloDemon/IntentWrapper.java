...
public class IntentWrapper {
    ...
    public static List<IntentWrapper> getIntentWrapperList() {
        if (sIntentWrapperList == null) {
            ...
            ResolveInfo oppoResolveInfo = MyApplication.getContext().getPackageManager().resolveActivity(oppoIntent, 0);
            if (oppoResolveInfo != null && !TextUtils.isEmpty(oppoResolveInfo.activityInfo.permission)) {
                // 新版oppo无法直接打开自启动页，只能打开上一级的隐私设置页，
    ...
    public static List<IntentWrapper> whiteListMatters(final Activity a, String reason) {
        ...
        if (showed.size() == 0) {// 说明没有可执行的intent 通知到设置界面
            MyApplication.getContext().sendBroadcast(new Intent(OtherBroadcast.NO_EXECUTABLE_INTENT));
        }
        return showed;
    }
    ...
    protected boolean doesActivityExists() {
        ...
        boolean ret = resolveInfo.activityInfo.exported
                && TextUtils.isEmpty(resolveInfo.activityInfo.permission);
        if (ret) {
            Log.e(TAG, "doesActivityExists() resolveInfo=" + JSON.toJSONString(resolveInfo));
        } else if (!TextUtils.isEmpty(resolveInfo.activityInfo.permission)) {
            Log.e(TAG, "doesActivityExists() resolveInfo=" + JSON.toJSONString(resolveInfo));
            Reporter.post("无权打开白名单设置页, permission=" + resolveInfo.activityInfo.permission);
        }
        return ret;
    }
    ...
    protected void startActivitySafely(Activity activityContext) {
        try {
            ...
            Reporter.post("打开白单名页失败， " + intent, e);
        }
    }
}
