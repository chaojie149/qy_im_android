...
import com.tongxin.caihong.ui.share.AuthorizationActivity;
import com.tongxin.caihong.ui.share.PayActivity;
import com.tongxin.caihong.ui.share.ShareConstant;
import com.tongxin.caihong.ui.share.ShareNearChatFriend;
...
import com.tongxin.caihong.view.DataLoadView;
...
public class DataDownloadActivity extends BaseActivity {
    ...
    public static void start(Context ctx, int isupdate, String password) {
        if (AccountSwitchHelper.getInSwitching() != null) {
            AccountSwitchHelper.finishSwitching();
            // 这里如果设置延迟会导致部分设备先展示桌面再打开页面，
            start(ctx, isupdate, password);
            return;
        }
    ...
    private void endDownload() {
        ...
        } else if (ShareConstant.IS_SHARE_P_COME) {
            intent = new Intent(DataDownloadActivity.this, PayActivity.class);
