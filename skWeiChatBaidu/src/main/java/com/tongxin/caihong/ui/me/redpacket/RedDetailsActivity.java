...
public class RedDetailsActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void showData() {
        ...
        if (isGroup && TextUtils.equals(packetEntity.getUserId(), coreManager.getSelf().getUserId())) {
            // 自己领取自己的红包，隐藏回复按钮
            red_reply_tv.setVisibility(View.GONE);
        }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.iv_title_left || v.getId() == R.id.red_back_tv) {
            ...
            DialogHelper.showLimitSingleInputDialog(this
                    ...
                    , "", getString(R.string.reply_red_thank) + getString(R.string.input_most_length, 10)
                    , 10
                    , text -> {
                        if (TextUtils.isEmpty(text)) {
                            ToastUtil.showToast(mContext, getString(R.string.tip_replay_empty));
                            return false;
                        }
                        replyRed(text);
                        return true;
