...
import com.tongxin.caihong.bean.company.Department;
import com.tongxin.caihong.bean.company.StructBean;
...
import com.tongxin.caihong.view.MarqueeTextView;
import com.tongxin.caihong.view.SelectCpyPopupWindow;
...
*/
            /**
             * 17029 iOS端创建公司，添加安卓用户进入公司，安卓端点击公司无法展开查看公司结构组织，安卓端创建公司添加iOS端用户，iOS端点击公司无法看到自己。
             * 巨坑，调试发现ios创建的公司，服务端返回的rootDpartId内的id根本就不是普通部门群的parentId，直接取根部门的id吧
             */
            if (data.get(i).getDepartments() != null) {
                rootDepartment = data.get(i).getDepartments().get(0).getId();
            }
            ...
            for (int j = 0; j < dps.size(); j++) {
                ...
                for (int z = 0; z < eps.size(); z++) {
                    ...
                    structBean.setHiding(eps.get(z).getHiding());
    ...
    class MyAdapter extends RecyclerView.Adapter<StructHolder> {
        ...
        public void onBindViewHolder(StructHolder holder, int position) {
            ...
            if (bean.isCompany() || bean.isDepartment()) {
                ...
                // 成员
                if (bean.getHiding() == 1) {
                    holder.ivInco.setImageResource(Constants.HIDING_AVATAR);
                    holder.tvTextName.setText(Constants.HIDING_NICKNAME);
                } else {
    ...
    class StructHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public StructHolder(View itemView, ItemClickListener listener) {
            ...
            ButtonColorChange.tintCheckBox(cbSelect2);
        }
