...
public class InviteVerifyActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        if (message.isDownload()) {
            ...
            // 待确认
            ButtonColorChange.colorChange(mContext, mSureTv);
    ...
    private void inviteFriend() {
        ...
        params.put("inviteUserId", message.getFromUserId());
        params.put(AppConstant.GROUP_ADD_STYLE, AppConstant.GROUP_INVITE_AUDIT);
                ...
                .execute(new BaseCallback<InviteResult>(InviteResult.class) {
                    ...
                    public void onResponse(ObjectResult<InviteResult> result) {
                        ...
                        if (Result.checkSuccess(mContext, result)) {
                            ...
                            for (int i = 0; i < mNeedUpdateVerifyMessage.size(); i++) {
                                // 更新消息内容 与 确认状态
                            ...
                            if (dataVerify != null && dataVerify.size() > 0) {
                                if (!TextUtils.isEmpty(inviteResult.getMsg())) {
                                    ...
                                    // 转换为提示消息，告知邀请者
                                    MessageUtil.generateTipMessage(mLoginUserId, friendId, inviteResult.getMsg(), true);
                                }
                            }
