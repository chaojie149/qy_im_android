        ...
        if (attentionUser == null) {// 服务器上不存在关系
            ...
            if (friend != null) {// 服务器上不存在关系，本地却有
                if (friend.getStatus() != Friend.STATUS_23
                        && friend.getStatus() != Friend.STATUS_UNKNOW
            ...
            if (friend == null) {// 本地不存在关系，那么就要插入一条好友记录
                ...
                if (attentionUser.getBlacklist() == 0) {
                    if (attentionUser.getIsBeenBlack() == 1) {
                        status = Friend.STATUS_19;
                    } else {
                        status = attentionUser.getStatus();
                    }
                } else {
                    status = Friend.STATUS_BLACKLIST;
                }
                if (status == friend.getStatus()) {
                    // do no thing
                } else {
                    if (status == Friend.STATUS_FRIEND && user.isSystem()) {
                        FriendDao.getInstance().updateFriendUserType(loginUserId, friendId, user.getUserType());
                    } else {
