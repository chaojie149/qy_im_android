...
import com.tongxin.caihong.util.SignatureUtil;
...
public class UpdateManger {
    ...
    public static void checkUpdate(Activity ctx, String updateUrl, String version, String explain) {
        ...
        try {
            ...
            manger.checkUpdateInfo(version, updateUrl, explain);
        } catch (Throwable t) {
            // 无论如何不能因为这个崩溃，
     ...
     * 检查签名与打包时是否一致，
     *
     * @return 返回true表示确实一致或者忽略检查，
     */
    private static boolean checkSignature(Context ctx) {
//        if (TextUtils.isEmpty(BuildConfig.SIGNATURE)) {
            return true;
//        }
//        String apkSign = SignatureUtil.getAppSignature(ctx);
//        return BuildConfig.SIGNATURE.equalsIgnoreCase(apkSign);
    }
    ...
    // 显示更新程序对话框，供主程序调用
    public void checkUpdateInfo(String serviceVersionCode, String apkUrls, String explain) {//添加检查服务器更新的代码
        ...
        if (mCurrentVersionCode.compareTo(serviceVersionCode) >= 0) {// 版本号不低 不需要更新
            ...
            apkUrl = apkUrls;
            showNoticeDialog(TextUtils.isEmpty(explain) ?
                            getStringById(R.string.application_version_update_down) : explain,
                    getStringById(R.string.application_update));
        }
    }
    ...
    private void showNoticeDialog(String desc, String title) {
        ...
        builder.setPositiveButton(MyApplication.getContext().getString(R.string.download), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                try {
                    if (checkSignature(mContext)) {
                        showDownloadDialog();
                    } else {
                        launchAppDetail(mContext, mContext.getPackageName(), null);
                    }
                } catch (Throwable t) {
