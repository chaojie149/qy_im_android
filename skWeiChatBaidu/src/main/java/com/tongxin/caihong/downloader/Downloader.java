...
public class Downloader {
    ...
    public void addDownload(String uri) {
        addDownload(uri, createViewAware(null), null, null);
    }

    public void addDownload(String uri, DownloadListener listener) {
        addDownload(uri, createViewAware(null), listener, null);
    }

    public void addDownload(String uri, DownloadProgressListener listener) {
        addDownload(uri, createViewAware(null), null, listener);
    }

    public void addDownload(String uri, DownloadListener listener, DownloadProgressListener listener2) {
        addDownload(uri, createViewAware(null), listener, listener2);
    }

    public void addDownload(String uri, ProgressBar progressBar, DownloadListener listener) {
        addDownload(uri, createViewAware(progressBar), listener, null);
    }

    public void addDownload(String uri, ProgressBar progressBar, DownloadListener listener, DownloadProgressListener listener2) {
        addDownload(uri, createViewAware(progressBar), listener, listener2);
    }

    private ViewAware createViewAware(View view) {
        if (view == null) {
            return new ViewAware(null);
        }
        ViewAware ret = (ViewAware) view.getTag(R.id.key_downloader_create_view_aware);
        if (ret == null) {
            ret = new ViewAware(view);
            view.setTag(R.id.key_downloader_create_view_aware, ret);
        }
        return ret;
    }

    public void cancelDisplayTaskFor(View view) {
        mEngine.cancelDisplayTaskFor(createViewAware(view));
    }
    ...
    public void addDownload(String uri, ViewAware viewAware, DownloadListener listener, DownloadProgressListener listener2) {
        ...
        if (localFile.exists()) {
            UploadCacheUtils.save(MyApplication.getContext(), uri, localFile.getPath());
