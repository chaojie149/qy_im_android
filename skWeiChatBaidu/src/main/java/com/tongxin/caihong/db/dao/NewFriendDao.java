...
public class NewFriendDao {
    ...
    public void deleteNewFriendById(String ownerId, String friendId) {
        try {
    ...
    public boolean createOrUpdateNewFriend(NewFriendMessage newFriend) {
        try {
            return new TransactionManager(newFriendDao.getConnectionSource()).callInTransaction(() -> {
                PreparedQuery<NewFriendMessage> preparedQuery = newFriendDao.queryBuilder()
...
*/
                ...
                if (existFriend != null) {
                    ...
                    // 时间也需要更新，用于排序，在updateNewFriendContent方法内判断是否非空吧
/*
                ...
                .execute(new BaseCallback<User>(User.class) {
                    ...
                    public void onResponse(ObjectResult<User> result) {
                        if (result.getResultCode() == 1 && result.getData() != null) {
                            ...
                            if (user.isSystem()) {// 公众号
                                FriendDao.getInstance().updateFriendUserType(CoreManager.requireSelf(MyApplication.getContext()).getUserId(),
                                        newFriend.getUserId(), user.getUserType());
                                CardcastUiUpdateUtil.broadcastUpdateUi(MyApplication.getContext());
