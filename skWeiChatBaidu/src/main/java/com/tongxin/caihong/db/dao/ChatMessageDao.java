...
import com.tongxin.caihong.db.UnlimitDaoManager;
...
public class ChatMessageDao {
            ...
            if (message.getType() != XmppMessage.TYPE_READ) {// 已读消息不更新
                if (message.isGroup()) {// 群组
                    if (message.getType() == XmppMessage.TYPE_TIP || TextUtils.isEmpty(message.getFromUserName())) {// 群组控制消息 || FromUserName为空
                        FriendDao.getInstance().updateFriendContent(ownerId, friendId, content, message);
                    } else {
                        FriendDao.getInstance().updateFriendContent(ownerId, friendId, message.getFromUserName() + " : " + content, message);
                    }
                } else {
                    String str;
                    if (message.getIsReadDel()
                        ...
                        if (TextUtils.equals(chatMessage.getFromUserId(), ownerId)) {
                            str = MyApplication.getContext().getString(R.string.tip_read_fire_msg);
                        } else {
                    ...
                    FriendDao.getInstance().updateFriendContent(ownerId, friendId, str, message);
                }
            }
            return true;
        } catch (SQLException e) {
    ...
    public boolean saveRoamingChatMessage(String ownerId, String friendId, ChatMessage message, boolean isGroup) {
        ...
        MessageUtil.handleRoamingSpecialMessage(message);
    ...
    public boolean saveNewSingleAnswerMessage(String ownerId, String friendId, ChatMessage message) {
        message.setObjectId(ChatTextClickPpWindow.DISABLE_RECORD);
        ...
        try {
            ...
            FriendDao.getInstance().updateFriendContent(ownerId, friendId, message.getContent(), message);
            // 保存这次的消息
            dao.create(message);
            return true;
        } catch (SQLException e) {
    ...
    public boolean deleteMessageByFromUserId(String ownerId, String friendId, String userId) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return false;
        }
    ...
    public boolean updateTranslation(String ownerId, String friendId, String packetId, String translation) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return false;
        }
        try {
            UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
            builder.updateColumnValue("translation", translation);
            builder.where().eq("packetId", packetId);
            dao.update(builder.prepare());
            return true;
        } catch (SQLException e) {
    ...
    public void updateMessageCompanyApply(String ownerId, String friendId, boolean isDownload, String content, long timeSend, String packetId) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return;
        }
     ...
     * 更新本地已读 单
     */
    public void updateMessageLocalRead(String ownerId, String friendId, String packetId, boolean state) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            Log.e(TAG, "更新已读失败:" + friendId);
            return;
        ...
        UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
        try {
            builder.updateColumnValue("localRead", state);
            if (state) { // 改变这条已读的是时候 加一个消息容错
                builder.updateColumnValue("messageState", ChatMessageListener.MESSAGE_SEND_SUCCESS);
            }
            builder.where().eq("packetId", packetId);
     ...
     * 更新已读 群
     */
    public void increaseMessageRead(String ownerId, String friendId, String packetId) {
        Log.d(TAG, "increaseMessageRead() called with: ownerId = [" + ownerId + "], friendId = [" + friendId + "], packetId = [" + packetId + "]");
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            Log.e(TAG, "更新已读失败:" + friendId);
            return;
        ...
        UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
        try {
            builder.updateColumnExpression("readPersons", "readPersons + " + 1);
            builder.where().eq("packetId", packetId);
            dao.update(builder.prepare());
        } catch (SQLException e) {
            Log.e(TAG, "更新已读失败:" + packetId);
     ...
     * 更新已读 群
     */
    public void setMessageRead(String ownerId, String friendId, String packetId, int count) {
        Log.d(TAG, "setMessageRead() called with: ownerId = [" + ownerId + "], friendId = [" + friendId + "], packetId = [" + packetId + "], count = [" + count + "]");
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            Log.e(TAG, "更新已读失败:" + friendId);
            return;
        ...
        UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
        try {
            builder.updateColumnValue("readPersons", count);
            builder.where().eq("packetId", packetId);
            dao.update(builder.prepare());
        } catch (SQLException e) {
            Log.e(TAG, "更新已读失败:" + packetId);
     ...
     * 更新消息content
     */
    public boolean updateMessageTypeContent(String ownerId, String friendId, String packetId, String content) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return false;
        }
    ...
    public boolean updateMessageBack(String ownerId, String friendId, String packetId, String name) {
        ...
        try {
            ...
            if (message != null) {
                if (message.getType() == XmppMessage.TYPE_TIP
                        && message.getFileSize() == XmppMessage.TYPE_BACK) {
                    // 群聊本地处理了一遍，收到撤回消息又处理了一遍，这种情况直接return.
                    return true;
                }
                if (message.getType() == XmppMessage.TYPE_TEXT
                    ...
                    builder.updateColumnValue("content", name + " " + MyApplication.getInstance().getString(R.string.other_withdraw)
                            + "," + MyApplication.getContext().getString(R.string.tip_edit_again));
                    // 将原消息内容保存在translation字段内
                    builder.updateColumnValue("translation", message.getContent());
                    builder.updateColumnValue("fileSize", XmppMessage.TYPE_BACK);
                } else {
                    builder.updateColumnValue("content", name + " " + MyApplication.getInstance().getString(R.string.other_withdraw));
                }
                builder.updateColumnValue("type", XmppMessage.TYPE_TIP);
     ...
     * 更新戳一戳/骰子状态
     */
    public void updateMessageAnimationState(String ownerId, String friendId, String packetId) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return;
        }
    ...
    public boolean updateChatMessageFileSize(String ownerId, String friendId, String packetId, long fileSize) {
        try {
            Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
            UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
            ChatMessage message = findMsgById(ownerId, friendId, packetId);
    ...
    /**
     * 请求有身份访客收到的评价消息是否已经评价的状态，
     */
    public boolean updateServiceGradeMessageStatus(String ownerId, String friendId, String packetId, boolean isDownload) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return false;
        }
     ...
     * 更新 群聊确认消息 的状态(不添加新字段了，以isDownload字段来标志，true 群组已确认 false 未确认)
     */
    public void updateMessageSeqNo(String ownerId, String friendId, String packetId, long seqNo) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return;
        }
        UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
        try {
            builder.updateColumnValue("seqNo", seqNo);
            builder.where().eq("packetId", packetId);
            dao.update(builder.prepare());
        } catch (SQLException e) {
            e.printStackTrace();
     ...
     * 查询并去重，
     */
    public List<ChatMessage> getSingleChatMessages(List<ChatMessage> messageList, String ownerId, String friendId, long time, int pageSize) {
        List<ChatMessage> newList = getSingleChatMessages(ownerId, friendId, time, pageSize);
        if (newList != null && messageList != null && !messageList.isEmpty()) {
            newList.removeAll(messageList);
        }
        return newList;
    }
    ...
    public List<ChatMessage> getSingleChatMessages(String ownerId, String friendId, long time, int pageSize) {
        ...
        try {
            ...
            builder.where().ne("type", XmppMessage.TYPE_READ).and().le("timeSend", time);
     ...
     * 查询并去重，
     */
    public List<ChatMessage> getOneGroupChatMessages(List<ChatMessage> messageList, String ownerId, String friendId, double time, int pageSize) {
        List<ChatMessage> newList = getOneGroupChatMessages(ownerId, friendId, time, pageSize);
        if (newList != null && messageList != null && !messageList.isEmpty()) {
            newList.removeAll(messageList);
        }
        return newList;
    ...
    /**
     * 新 查询与friendId的聊天记录(漫游任务参与sql)
     */
    private List<ChatMessage> getOneGroupChatMessages(String ownerId, String friendId, double time, int pageSize) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return null;
        }
...
*/
        try {
            if (mLastMsgRoamTask == null) {
                ...
                builder.where().ne("type", XmppMessage.TYPE_READ)
                        .and().ne("isExpired", 1)
                        .and().ge("seqNo", mLastMsgRoamTask.getEndSeqNo())
                        .and().le("timeSend", time);
                builder.orderBy("timeSend", false);
                builder.orderBy("_id", false);
                builder.limit((long) pageSize);
            ...
            messages = dao.query(builder.prepare());
            if (mLastMsgRoamTask != null && messages != null && messages.size() > 0) {
                // 任务不为空且任务内消息的条数大于一次漫游能获取到的消息条数时
                // seq为0且timeSend大于任务最后一条消息timeSend的消息将无法查出
                // 这种情况下我们在查一遍数据库，找到seq为0且timeSend大于messages最后一条消息timeSend的消息
                List<ChatMessage> messages2 = getOneGroupChatMessages2(ownerId, friendId, messages.get(0).getDoubleTimeSend(), pageSize);
                if (messages2 != null && messages2.size() > 0) {
                    messages.addAll(0, messages2);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private List<ChatMessage> getOneGroupChatMessages2(String ownerId, String friendId, double time, int pageSize) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return null;
        }
     ...
     */
    public List<ChatMessage> getCourseChatMessage(String ownerId, String friendId, double startTime, double endTime) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return null;
        }
     ...
     * 根据指定字段获取消息
     */
    public ChatMessage findMsgByAppointField(String ownerId, String friendId, String fieldName, String fieldValue) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return null;
        }
     ...
     * 查询某条消息的已读成员列表
     */
    public List<ChatMessage> queryFriendsByReadList(String loginUserId, String roomId, String packetId, int pageIndex, int pageSize) {
        ...
        try {
            ...
            builder.orderBy("timeSend", false);
            builder.limit((long) pageSize).offset((long) (pageIndex * pageSize));
     ...
     * 查询某人是否已读某条消息，
     */
    public boolean checkUserReadOrNot(String loginUserId, String roomId, String packetId, String userId) {
        Dao<ChatMessage, Integer> dao = getDao(loginUserId, roomId);
        QueryBuilder<ChatMessage, Integer> builder = dao.queryBuilder();

        try {
            return builder.where().eq("type", XmppMessage.TYPE_READ).and().eq("content", packetId)
                    .and().eq("fromUserId", userId)
                    .queryForFirst() != null;
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List<Friend> queryChatMessageByContent(Friend friend, String content) {
        ...
        try {
            builder.where().in("type", XmppMessage.TYPE_TEXT, XmppMessage.TYPE_REPLAY)
                    .and().eq("isReadDel", "0")
                    .and().ne("isExpired", 1)
