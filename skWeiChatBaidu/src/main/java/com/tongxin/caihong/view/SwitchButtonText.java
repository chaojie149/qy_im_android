 ...
 * SwitchButton.
 */
public class SwitchButtonText extends View implements Checkable {
    private static int DEFAULT_WIDTH = dp2pxInt(58);
    private static int DEFAULT_HEIGHT = dp2pxInt(36);
    ...
    public SwitchButtonText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public SwitchButtonText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public SwitchButtonText(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        //加上设置默认宽高的属性
        DEFAULT_WIDTH = dp2pxInt(optInt(typedArray,
                R.styleable.SwitchButton_kswThumbWidth,
                58));
        DEFAULT_HEIGHT = dp2pxInt(optInt(typedArray,
    ...
    private void drawButton(Canvas canvas, float x, float y) {
        ...
        // drawCircle，x，y为圆圈圆点的坐标，
        // drawText，y的位置为文字的基线位置
        Paint textPaint = new Paint();
        textPaint.setAntiAlias(true);
        textPaint.setTextSize(dp2px(13));
        textPaint.setColor(getResources().getColor(R.color.live_stock_not_enough1));
        canvas.drawText("弹", (float) (x - buttonRadius / 1.6), (float) (y + buttonRadius / 2.8), textPaint);
    }
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(SwitchButtonText view, boolean isChecked);
    }
