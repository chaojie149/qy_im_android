...
import com.tongxin.caihong.audio.IMRecordController;
import com.tongxin.caihong.audio.RecordListener;
...
import com.tongxin.caihong.util.InputManager;
...
import com.tongxin.caihong.util.input.KeyboardPanelUtil;
...
public class ChatBottomView extends LinearLayout implements View.OnClickListener {
    ...
    private AutoCompleteTextView mChatEdit;
    ...
    private void init(Context context) {
        ...
        mChatEdit = findViewById(R.id.chat_edit);
        ...
        mChatEdit.addTextChangedListener(new TextWatcher() {
            ...
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
                removeAtBefore(s, start, count, after);
            }
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                removeAtAfter(s, start, before, count);
                if (s instanceof Editable) {
                    AtCopyHelper.rebuildAtUserList(roomId, (Editable) s, start, count, (AtCopyHelper.AddAtUserListener) (span, atBean) -> {
                        Map<ForegroundColorSpan, AtBean> atBeanMap = mBottomListener.getAtBeanMap();
                        if (atBeanMap != null) {
                            atBeanMap.put(span, atBean);
                        }
                    });
                }
            ...
            private void removeAtBefore(CharSequence s, int start, int count, int after) {
                if (count <= after || !(s instanceof Editable)) {
                    return;
                }
                Map<ForegroundColorSpan, AtBean> atBeanMap = mBottomListener.getAtBeanMap();
                if (atBeanMap == null || atBeanMap.isEmpty()) {
                    return;
                }
                Editable editableText = (Editable) s;
                ForegroundColorSpan[] spans = editableText.getSpans(start + after, start + count, ForegroundColorSpan.class);
                if (spans == null || spans.length == 0) {
                    return;
                }
                for (ForegroundColorSpan span : spans) {
                    if (atBeanMap.remove(span) == null) {
                        continue;
                    }
                    int spanStart = editableText.getSpanStart(span);
                    int spanEnd = editableText.getSpanEnd(span);
                    editableText.removeSpan(span);
                    if (spanMinStart == -1 || spanStart < spanMinStart) {
                        spanMinStart = spanStart;
                    }
                    if (spanMaxEnd == -1 || spanEnd > spanMaxEnd) {
                        spanMaxEnd = spanEnd;
                    }
                }
            }

            private void removeAtAfter(CharSequence s, int start, int before, int count) {
                if (spanMinStart == -1 || spanMaxEnd == -1 || count >= before || !(s instanceof Editable)) {
                    return;
                }
                Editable editableText = (Editable) s;
                int spanStart = spanMinStart;
                int spanEnd = spanMaxEnd;
                spanMinStart = -1;
                spanMaxEnd = -1;
                editableText.replace(spanStart, spanEnd - (before - count), "");
            }
    ...
    private void changeChatFaceView(boolean show) {
        ...
        if (show) {
            ...
            androidx.core.view.ViewCompat.setBackgroundTintList(mEmotionBtn, ColorStateList.valueOf(SkinUtils.getSkin(mContext).getAccentColor()));
        } else {
            ...
            switch (v.getId()) {
                ...
                case R.id.voice_img_btn:
                    if (!PermissionUtil.checkAndAutoRequestPermission((AppCompatActivity) mContext, Short.MAX_VALUE, () -> {
                        onClick(v);
                    }, null, PermissionUtil.getRecordAudioPermissions())) {
                        return;
                    }
...
//                        showKeyboard();
    ...
    public void showMoreSelectMenu(boolean isShow) {
        ...
        if (isShow) {
            ...
            if (isGroup) {
                boolean isAllowSecretlyChat = PreferenceUtils.getBoolean(mContext, Constants.IS_SEND_CARD + roomJid, true);
                if (!isAllowSecretlyChat && (mRole != 1 && mRole != 2)) {
                    // 禁止群成员私聊时禁止收藏，以免通过收藏添加好友，
                    lLMoreSelect.findViewById(R.id.more_select_collection_iv).setVisibility(View.GONE);
                }
            }
        } else {
    ...
    public void addRecordListener() {
        if (mRecordController != null) {
            mRecordController.addListener();
        }
    }

    public void removeRecordListener() {
        if (mRecordController != null) {
            mRecordController.removeListener();
        }
    }
    ...
    public void setReplay(ChatMessage chatMessage) {
        ...
        mChatEdit.setFocusableInTouchMode(true);
        mChatEdit.requestFocus();
        mChatEdit.postDelayed(() -> {
            InputMethodManager inputManager = (InputMethodManager) mChatEdit.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
            inputManager.showSoftInput(mChatEdit, 0);
    ...
    public void setAutoFillData(List<Question> data) {
        mChatEdit.setAdapter(new SearchAutoFillAdapter(mContext, android.R.layout.simple_list_item_1, data));
        mChatEdit.setOnItemClickListener((parent, view, position, id) -> {
            mBottomListener.sendText(mChatEdit.getText().toString(), true);
            mChatEdit.setText("");
        });
    }
    ...
    public interface ChatBottomListener {
        default Map<ForegroundColorSpan, AtBean> getAtBeanMap() {
            return null;
        }
        ...
        default void sendText(String text, boolean isQuestion) {
        }
    ...
    private static class SearchAutoFillAdapter extends ArrayAdapter<String> {
        private ArrayList<Question> mOriginalValues;

        public SearchAutoFillAdapter(@NonNull Context context, int resource, @NonNull List<Question> objects) {
        ...
        public Filter getFilter() {
            return new SearchAutoFillFilter();
        }

        private class SearchAutoFillFilter extends Filter {
            @Override
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (TextUtils.isEmpty(constraint)) {
                    ...
                    for (Question question : mOriginalValues) {
                        /*
                         * 问题内容模糊匹配，问题包含当前输入，
                         * 关键字模糊匹配，当前输入包含关键字，
                         */
                        if (question.getQuestion().contains(content)) {
