...
public class ChatMessage extends AbstractMessage {
     ...
     * @param isGroup
     */
    private static void encryptContent(com.tongxin.caihong.bean.message.ChatMessage chatMessage, boolean isGroup) {
     ...
     * @return
     */
    public static ChatMessage toSocketMessage(String mLoginUserId, com.tongxin.caihong.bean.message.ChatMessage message, boolean isNewFriendMsg) {
        com.tongxin.caihong.bean.message.ChatMessage chatMessage = message.clone(message.isGroup());
        ...
        if (!isNewFriendMsg && MessageUtil.isNeedSeqNoMessage(chat.getType())) {
        ...
        if (chat.getFromUserId().equals(chat.getToUserId())) {
            ...
            if (!TextUtils.isEmpty(chatMessage.getToId())
                    && !TextUtils.equals(chatMessage.getToUserId(), chatMessage.getToId())) {
                // 有身份访客发送给公众号的消息要转到对应客服，
                head.setTo(chatMessage.getToId());
            } else {
        ...
        if (!TextUtils.isEmpty(chatMessage.getServiceId())) {
            chat.setSrvId(chatMessage.getServiceId());
        }
    ...
    public String toString() {
        ...
        object.put("seqNo", this.seqNo);
        object.put("subType", this.subType);
        object.put("messageHead", this.messageHead);
        object.put("srvId", this.srvId);
     ...
     * @return
     */
    public com.tongxin.caihong.bean.message.ChatMessage toSkMessage(String loginId) {
        com.tongxin.caihong.bean.message.ChatMessage chat = new com.tongxin.caihong.bean.message.ChatMessage();

        if (this.seqNo > 0) {
            chat.setSeqNo(this.seqNo);
            Log.e("msg", "为消息设置序号：" + chat.getSeqNo());
        } else {
